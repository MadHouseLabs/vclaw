...
impl TmuxController {
    ...
    pub async fn send_keys(&self, target: &str, keys: &str, enter: bool) -> Result<()> {
        ...
        self.execute_args(&["send-keys", "-t", target, "-l", keys])
            .await?;
        if enter {
            self.execute_args(&["send-keys", "-t", target, "Enter"])
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn configure_session(&self) -> Result<()> {
        ...
        self.execute_args(&[
            ...
            &self.session_name,
            "status-left",
            "#[fg=colour16,bg=colour39,bold] \u{25c6} vclaw #[default] ",
        ])
        .await?;
        self.execute_args(&[
            "set-option",
        ...
        self.execute_args(&["bind-key", "-T", "root", "F12", "run-shell", ctl_voice])
            .await?;
        self.execute_args(&["bind-key", "-T", "root", "M-m", "run-shell", ctl_mute])
            .await?;
        // Interrupt is handled by F12 toggle (press while speaking/processing).
        // Escape in prefix table as explicit fallback.
        self.execute_args(&["bind-key", "Escape", "run-shell", ctl_interrupt])
            .await?;

        // Prefix table
        self.execute_args(&["bind-key", "Space", "run-shell", ctl_voice])
            .await?;
        self.execute_args(&[
            "bind-key",
            "C",
